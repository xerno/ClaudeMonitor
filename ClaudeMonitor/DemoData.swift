...
enum DemoData {
    ...
    private static func scenario1() -> (UsageResponse, StatusSummary) {
        let usage = UsageResponse(
            fiveHour: UsageWindow(utilization: 42, resetsAt: Date().addingTimeInterval(2.7 * 3600)),
            sevenDay: nil,
            sevenDaySonnet: nil
        )
    ...
    private static func scenario2() -> (UsageResponse, StatusSummary) {
        let usage = UsageResponse(
            fiveHour: UsageWindow(utilization: 74, resetsAt: Date().addingTimeInterval(1.4 * 3600)),
            sevenDay: UsageWindow(utilization: 61, resetsAt: Date().addingTimeInterval(4.2 * 86400)),
            sevenDaySonnet: nil
        )
        let status = StatusSummary(
            components: [
